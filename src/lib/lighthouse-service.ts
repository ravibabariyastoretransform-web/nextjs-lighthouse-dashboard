...
export async function runLighthouseAudit(url: string, forceSimulated = false): Promise<AuditReport> {
    ...
    try {
        ...
        const command = `npx lighthouse ${checkUrl} --preset=desktop --output=json --output-path=${tempOutPath} --chrome-flags="--headless --no-sandbox --disable-gpu --disable-dev-shm-usage --disable-extensions --incognito" --quiet`;
